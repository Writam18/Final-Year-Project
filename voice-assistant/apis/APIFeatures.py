...
__MONTHS = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october',
            'november', 'december']
...
def noteMaker(text):
    ...
    if "yes" in (get_audio()).split():
        ...
        speak(
            "ok sir!! no problem i'll save it myself with a date and time stamp so that you may know when it's created")
...
# SEARCH DAILY NEWS FOR A PARTICULAR TOPIC
...
def findNews():
    ...
    while True:
        ...
        text = get_audio()
        if "no" in text.split() or "ERROR" in text:
            speak("ok duly noted")
            break
        googlenews.clear()
